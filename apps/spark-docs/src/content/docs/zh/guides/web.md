...
侧栏顶部保留“新对话”，底部固定“设置”。中间区域独立滚动，按工作空间分组展示最近会话，每组先显示五条，可“展开显示”更多；较旧的当前会话仍保留在列表中。点击工作空间标题进入对应页面，“全部对话”打开完整列表。页面切换、窗口重新获得焦点及当前会话活动状态变化时，侧栏重新读取 daemon 投影，不自行推断运行状态。手机菜单使用相同分组，选择会话后自动关闭。
